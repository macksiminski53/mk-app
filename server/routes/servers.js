...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    members: members.map((m) => ({ ...m, isUltra: !!m.isUltra })),
  });
}));
...
router.post('/:id/members', asyncHandler(async (req, res) => {
  ...
  const target = await db.prepare('SELECT id, username, avatar_color as avatarColor, avatar_url as avatarUrl, is_ultra as isUltra FROM users WHERE username = ?').get(clean);
  ...
  res.json({ id: target.id, username: target.username, avatarColor: target.avatarColor, avatarUrl: target.avatarUrl, isUltra: !!target.isUltra });
}));
